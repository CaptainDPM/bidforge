...
const C = {
  ...
  dim: "#444", green: "#4caf7d", red: "#e05c5c", blue: "#7b8fc8",
};
    ...
    <div style={{ ...S.app, alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: 420, padding: "0 20px", boxSizing: "border-box" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <div style={{ fontSize: 13, color: C.dim }}>{sub}</div>
        </div>

        <div style={S.card}>
          <Err msg={err} />
          {success && (
            <div style={{ background: "rgba(76,175,125,0.08)", border: "1px solid rgba(76,175,125,0.2)", borderRadius: 8, padding: "10px 13px", color: C.green, fontSize: 13, marginBottom: 14 }}>
              {success}
            </div>
          )}
          ...
          {mode === "register" && (
            <>
              <div style={{ marginBottom: 14 }}>
                ...
                <input style={S.inp} placeholder="Derek Marquart" value={fullName} onChange={(e) => setFullName(e.target.value)} onKeyDown={onKey} />
              </div>
              <div style={{ marginBottom: 14 }}>
                <label style={S.label}>Company / Organization</label>
                <input style={S.inp} placeholder="Marquart IT Solutions LLC" value={orgName} onChange={(e) => setOrgName(e.target.value)} onKeyDown={onKey} />
              </div>
            </>
          )}
          ...
          {(mode === "login" || mode === "register" || mode === "forgot") && (
            <div style={{ marginBottom: 14 }}>
              ...
              <input style={S.inp} type="email" placeholder="you@company.com" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={onKey} />
            </div>
          )}
          ...
          {(mode === "login" || mode === "register") && (
            <div style={{ marginBottom: mode === "login" ? 8 : 20 }}>
              ...
              <input style={S.inp} type="password" placeholder="Min 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={onKey} />
            </div>
          )}

          {mode === "login" && (
            <div style={{ textAlign: "right", marginBottom: 16 }}>
              <button onClick={() => go("forgot")} style={{ background: "none", border: "none", cursor: "pointer", color: C.dim, fontSize: 12 }}>
                Forgot password?
              </button>
            </div>
          )}
          ...
          {mode === "reset" && (
            <div style={{ marginBottom: 20 }}>
              ...
              <input style={S.inp} type="password" placeholder="Min 8 characters" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} onKeyDown={onKey} autoFocus />
            </div>
          )}
        ...
        <div style={{ textAlign: "center", fontSize: 13, color: C.dim, marginTop: 14 }}>
...
export default function App() {
  ...
  const loadProjects = useCallback(async () => {
    ...
    try { const d = await api.projects.list(); setProjects(d.projects || []); }
    catch {}
  ...
  useEffect(() => {
    api.projects.solicitationTypes().then((d) => setSolGroups(d.groups || {})).catch(() => {});
  }, []);
  ...
  const run = () => {
    ...
    if (rfpFile) fd.append("rfp", rfpFile);
    else fd.append("rfpText", rfpText);
  ...
  const Header = ({ children }) => (
    <header style={S.hdr}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ fontSize: 12, color: C.dim }}>{user.full_name} · <span style={{ color: C.gold }}>{user.plan}</span></span>
        <button onClick={() => setShowTeam(true)} style={{ ...S.btn, ...S.ghost, padding: "5px 10px", fontSize: 11 }}>
  ...
  if (screen === "home") return (
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        {projsLoading ? (
          <div style={{ textAlign: "center", padding: 60, color: C.dim }}>Loading projects…</div>
        ) : projects.length === 0 ? (
          <div style={{ ...S.card, textAlign: "center", padding: "60px 24px" }}>
            ...
            <div style={{ fontSize: 13, color: C.dim, marginBottom: 24 }}>Upload your first RFP to get started.</div>
              ...
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: C.surface, borderRadius: 10, border: `1px solid ${C.border}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: C.dim }}>{p.company_name} · {p.req_count} reqs · {p.created_by_name} · {new Date(p.created_at).toLocaleDateString()}
                  ...
                  <div style={{ textAlign: "center", flexShrink: 0, minWidth: 44 }}>
                    ...
                    <div style={{ fontSize: 10, color: C.dim }}>score</div>
                  </div>
  ...
  if (screen === "upload") return (
    <div style={S.app}>
      ...
      <div style={{ ...S.main, maxWidth: 720 }}>
        ...
        <div style={S.card}>
          {/* Solicitation type — full width, first field */}
          <div style={{ marginBottom: 18 }}>
            ...
            {solGroups && Object.values(solGroups).flat().find((i) => i.value === solType) && (
              <div style={{ fontSize: 11, color: C.dim, marginTop: 5, paddingLeft: 2 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div>
              ...
              <input style={S.inp} placeholder="e.g. VETS 2 — Boeing Sub" value={projName} onChange={(e) => setProjName(e.target.value)} />
            </div>
            <div>
              <label style={S.label}>Company Name</label>
              <input style={S.inp} placeholder={user.org_name} value={coName} onChange={(e) => setCoName(e.target.value)} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
            ...
            <div>
              <label style={S.label}>Past Performance <span style={{ color: C.dim, fontWeight: 400, textTransform: "none", letterSpacing: 0, fontSize: 11 }}>(optional)</span></label>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 40 }}>
        <div style={{ maxWidth: 560, width: "100%" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 12, color: C.dim }}>{stepLabel}</div>
          </div>
          {streamText && (
        ...
        <div style={{ ...S.main, maxWidth: 1180 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: 8, marginBottom: 16 }}>
            {[{l:"Score",v:`${score}%`,c:scoreColor},{l:"Total",v:reqs.length,c:"#aaa"},{l:"Fully Met",v:cnt.full,c:C.green},{l:"Partial",v:cnt.partial,c:C.gold},{l:"Gaps",v:cnt.none,c:C.red},{l:"TBC",v:cnt.tbc,c:C.blue}].map(({l,v,c})=>(
              <div key={l} style={S.stat}>
                ...
                <div style={{fontSize:10,color:C.dim,marginTop:4,letterSpacing:"0.06em",textTransform:"uppercase"}}>{l}</div>
              </div>
            ))}
          </div>
          ...
          {!disclaimerDismissed && (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 16px", background: "rgba(123,143,200,0.07)", border: "1px solid rgba(123,143,200,0.2)", borderRadius: 10, marginBottom: 16 }}>
              <Ic d={P.warn} size={15} color={C.blue} style={{ flexShrink: 0, marginTop: 1 }} />
              <div style={{ flex: 1, fontSize: 12, color: "#8fa0cc", lineHeight: 1.6 }}>
                <strong style={{ color: C.blue }}>AI-Assisted Content — Review Required.</strong> BidForge helps accelerate your proposal writing process. All AI-generated content — including requirements, compliance assessments, and proposal text — must be reviewed and verified by your team before submission. Your organization is solely responsible for the accuracy, completeness, and regulatory compliance of any proposal submitted.
              </div>
              <button onClick={() => setDisclaimerDismissed(true)} style={{ background: "none", border: "none", cursor: "pointer", color: C.dim, padding: "2px 4px", flexShrink: 0 }} title="Dismiss">
                <Ic d={P.x} size={13} />
              </button>
            </div>
          )}
          ...
          {tab === "matrix" && (
            <div style={S.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                ...
                <span style={{fontSize:11,color:C.dim}}>{reqs.length} requirements</span>
              </div>
              <div style={{overflowX:"auto",borderRadius:8,border:`1px solid ${C.border}`}}>
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  <thead>
                    <tr>{["ID","Requirement","Category","Priority","Status","Response Strategy","Owner"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {reqs.map((r,i)=>{
                      ...
                      return(
                        <tr key={r.id||i} style={{background:i%2?"rgba(255,255,255,0.01)":"transparent"}}>
                          <td style={{...S.td,color:C.dim,fontFamily:"monospace",fontSize:11}}>R{String(i+1).padStart(3,"0")}</td>
                          ...
                          <td style={{...S.td,color:"#666",lineHeight:1.5,maxWidth:220}}>{r.response_strategy}</td>
                          <td style={{...S.td,color:C.dim}}>{r.owner||"TBD"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          {tab === "proposal" && (
            <div style={S.card}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 13px", background: "rgba(200,169,110,0.05)", border: "1px solid rgba(200,169,110,0.15)", borderRadius: 8, marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 11, color: "rgba(200,169,110,0.8)", lineHeight: 1.5 }}>
                  This is an AI-generated draft. Review all content carefully and verify accuracy before including in any submission. Your team is responsible for the final proposal.
                </span>
              </div>
              <div style={{background:C.bg,borderRadius:8,padding:"20px 24px",maxHeight:600,overflowY:"auto",border:`1px solid ${C.border}`}}>
                {proposal.split("\n").map((line,i)=>{
                  ...
                  return<p key={i} style={{color:"#777",lineHeight:1.8,marginBottom:4,fontSize:13}}>{line}</p>;
                })}
              </div>
            </div>
          )}
            ...
            <div style={S.card}>
              ...
              <div style={{background:C.bg,borderRadius:8,padding:"20px 24px",border:`1px solid ${C.border}`,marginBottom:14}}>
                {execSummary.split("\n").filter(Boolean).map((p,i)=>(
                  <p key={i} style={{color:"#888",lineHeight:1.8,marginBottom:12,fontSize:14}}>{p}</p>
                ))}
              </div>
            </div>
          )}
          ...
          <button onClick={goHome} style={{...S.btn,...S.ghost,marginTop:4}}>
            <Ic d={P.home} size={13}/> Back to Projects
          </button>
        </div>
      </div>
    );
    ...
    <div style={S.app}>
      ...
      <div style={{ ...S.main, maxWidth: 780 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          ...
          {user.plan !== "trial" && (
            <button onClick={async () => { const d = await api.billing.portal(); window.location.href = d.url; }} style={{ ...S.btn, ...S.ghost }}>
              Manage Subscription
            </button>
          )}
        </div>
          ...
          {[
            ...
            return (
              <div key={key} style={{ background: highlight ? "rgba(200,169,110,0.05)" : C.surface, border: `1px solid ${highlight ? "rgba(200,169,110,0.3)" : C.border}`, borderRadius: 13, padding: 22, display: "flex", flexDirection: "column", position: "relative" }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 700, color: C.gold, fontFamily: "'Crimson Pro',Georgia,serif", marginBottom: 4 }}>{price}<span style={{ fontSize: 13, color: C.dim, fontFamily: "inherit", fontWeight: 400 }}>{price !== "Custom" ? "/mo" : ""}</span></div>
                <div style={{ fontSize: 12, color: C.dim, marginBottom: 16 }}>{analyses} analyses · {seats} seats</div>
                <div style={{ flex: 1, marginBottom: 18 }}>
                  {features.map((f) => (
                    <div key={f} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, fontSize: 12, color: "#888" }}>
                ...
                {isCurrent ? (
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const d = await api.billing.checkout(key);
        ...
        {user.plan === "trial" && (
          <div style={{ padding: "14px 18px", background: "rgba(123,143,200,0.07)", border: "1px solid rgba(123,143,200,0.2)", borderRadius: 10, fontSize: 13, color: "#8fa0cc" }}>
            <strong>You're on the free trial.</strong> You have {user.api_limit - user.api_calls_this_month} analyses remaining. Upgrade anytime — no data is lost when you upgrade.
          </div>
        )}

        <button onClick={goHome} style={{ ...S.btn, ...S.ghost, marginTop: 20 }}>
          <Ic d={P.home} size={13} /> Back to Projects
        </button>
      </div>
    </div>
...
function TeamModal({ user, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: 28, width: "100%", maxWidth: 540, maxHeight: "80vh", overflow: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ ...S.h2, marginBottom: 0 }}>Team Members</div>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: C.muted }}><Ic d={P.x} size={16} /></button>
        </div>
          ...
          <div style={{ marginBottom: 20 }}>
            {team.map((m) => (
              <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: `1px solid ${C.border}` }}>
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(200,169,110,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: C.gold }}>{(m.full_name || "?")[0].toUpperCase()}</span>
                </div>
                <div style={{ flex: 1 }}>
        ...
        {["owner", "admin"].includes(user.role) && (
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.muted, marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.06em", fontSize: 11 }}>Invite Team Member</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
              <input style={S.inp} placeholder="Full name" value={inviteName} onChange={(e) => setInviteName(e.target.value)} />
              <input style={S.inp} placeholder="Email" type="email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} />
            </div>
            {inviteErr && <Err msg={inviteErr} />}
            {inviteResult && <div style={{ fontSize: 12, color: C.green, marginBottom: 8 }}>Invited! Temp password: <code style={{ background: C.bg, padding: "2px 6px", borderRadius: 4 }}>{inviteResult.tempPassword}</code></div>}
            <button onClick={invite} style={{ ...S.btn, ...S.gold, width: "100%", justifyContent: "center" }}>
